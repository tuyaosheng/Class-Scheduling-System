...
注意：这里说的"合排"特指**把多个年级塞进同一个 CP-SAT 模型统一求解**——「多年级操作流程重构」子项目9的"求解阶段跨年级避让"不是这个，是求解某年级前把其它年级已经排好的结果换算成本年级的 `forbid_slots` 规则、仍然各自独立跑各自的单年级 CP-SAT 模型（见上一节），不共享 `class_id`/`compiler.x` 键空间，不受这里两条缺口影响。真要做"多年级合并进一个模型求解"才需要先做这里的重构。
