...
现状：体比、体选目前是「教务固定占位」课程（见坑 6），不生成任务、不进求解器，这条豁免逻辑（session 模型）暂时用不上，但代码与测试都保留着——一旦这两门课重新纳入求解范围，或将来别的合班课出现，直接复用。
...
- 操场同时可容纳班数（配置暂留空 = 不限制）。**现状已不影响求解**：体比、体选已改为教务固定占位（坑 6），不进求解器，操场容量在系统求解范围内目前只有普通体育课一门，谈不上并发。若未来把体比/体选重新纳入求解范围，这个口径问题会再次出现——到时再问，**问之前先说清口径**：场地占用按 **session**（同教师同一门合班课整门计 1 处）而非按班计。`rules.py` 里 `venue_capacity` 的描述文案还写着「个班」，一并改掉再问。
