...
| 7. 跨年级统一校验 + 全部导出 | 导出前按真实钟点比对教师跨年级冲突；"导出全部课表" | **已完成**（2026-08-28）——新增 `scheduler/core/cross_grade.py::find_cross_grade_conflicts(entries)`：按教师姓名 + 真实钟点区间（换算成分钟数比较区间重叠）判断跨年级冲突，不比较"第几节"；日历没配 `clock_times` 的年级直接跳过并在 `skipped_grades` 里报出来，不静默漏检。API 新增 `POST /api/export/all/check`（只校验）和 `POST /api/export/all`（校验+导出，服务端强制重新校验一遍，不信任前端"已经 check 过"，冲突时 400 拒绝、干净时返回一个含各年级 xlsx 的 zip）。前端新增 `ExportAllSettings.vue` 挂在第9步：按年级选历史求解任务+候选方案序号，"校验"按钮显示冲突/跳过年级，"导出全部"触发浏览器下载。**开发过程中发现并修复一个真实存在的 bug**（不是这次新引入的）：`ws.py` 的 `_run_job`（`/api/solve` 这条真正被产品使用的求解路径）重建 `Dataset` 时完全没有传 `calendar=` 参数，一直在用 `models.py` 的硬编码默认日历（9节/天、无 `clock_times`、无 `reserved_slots`）——`cli.py`、`import_teaching_table` 等路径一直是对的（都显式调用 `cfg.calendar_of(grade)`），只有 `_run_job` 从 `teaching.yaml` 原始字典直接重建 Dataset 这条路径漏了，因为它不经过任何一个已经接入日历参数化的 importer 函数。对初三而言形状恰好一样（9节/午休5节后）掩盖了问题；对七年级（8节/天）会静默用错误的网格求解，且 reserved_slots 挖空和真实钟点表全部失效——这也是本子项目的校验一开始对真实历史 job 全部命中"skipped_grades"的原因（那些 job 是修复前求解的，快照里没有 clock_times）。已修复为 `Dataset(..., calendar=cfg.calendar_of(data['grade']))`，全套 432 项后端测试仍然全绿（说明之前的测试从未真正覆盖过这条路径的日历正确性）。测试：`test_cross_grade.py`（5项，专门构造两个作息形状不同的日历，确保"同一个节次编号在不同年级不是同一个真实时间"这件事被真正验证到，不是巧合对齐）、`test_api_export_all.py`（6项，含跨年级冲突检测、拒绝导出、成功导出zip、job/年级不匹配校验）、`test_api_solve.py` 新增一条专门针对 `_run_job` 日历修复的回归测试。浏览器实测确认真实初三历史 job（2026-08-24 求解，早于本次修复）被正确标记为"没有配置真实钟点表"，校验仍报"通过"（单年级、没有冲突可比较），点"导出全部"成功触发下载——整个链路在真实数据上走通 |
