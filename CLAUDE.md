...
中小学（初中）排课系统。**M1–M6 已完成**（领域模型/配置/Excel 导入/中文规则解析 → 规则 DSL/约束编译/求解/场地 → 校验器/预检/冲突集 → 软约束优化/多候选方案 → Web 界面全部视图与拖拽 → AI 审核）。**M7 核心（三年级合排）已完成，打包 exe 未开始**——2026-09-01 新增 `scheduler/core/merge_solver.py`，联合求解多个年级并按真实钟点保证跨年级教师/共享场地不冲突，见下方「M7：三年级合排」一节。2026-08-28 起在此基础上进行「多年级操作流程重构」（年级管理、按年级维护课程/规则、任课表与排课说明分工、界面导航重排、AI 供应商切换到 OpenAI 兼容协议），最初规划的 8 个子项目（年级管理与作息表导入、课程/学科系按年级维护、单双周配对、任课表导入编辑、排课规则导入、拖拽调整按违规类型着色、跨年级统一校验与全部导出、AI 供应商抽象）均已完成，随后又追加了子项目9（把子项目7的"导出前事后校验"改成"求解阶段就避开"，外加规则设置页面的英文翻译），详见下方「实施计划」表与「多年级操作流程重构」一节。
...
年级日历参数化那次分支审查还留了三处已知的遗留缺口——**2026-09-01 已全部处理完**，见「去掉硬编码9节/天」一节：`scheduler/ai/rule_parser.py` 的 AI 规则解析提示词硬编码 9 节/天（已改成按传入 calendar 动态生成）；`solver.py` 的 `Placement.day`/`.period` 属性是死代码，同样硬编码（已删除）；`ScheduleGrid.vue` 曾经硬编码 `PERIODS_PER_DAY=9`，早前拖拽功能已经把它改成了可选 `days`/`periodsPerDay` props，但当时调用方还没接入真实日历数据（`CandidateTabs.vue` 现在会取真实作息表传下去）。
...
- **多年级求解**：各年级独立求解（现有单年级引擎不用改）。~~导出前必须跑一次跨年级统一校验，校验不通过不能导出~~——**2026-08-28 改成求解阶段就避开**（子项目9，见下方实施计划表）：求解某年级前，把其它年级"最近一次求解出的第一个候选方案"当作既定事实，按教师姓名 + 真实钟点区间换算成本年级的 `forbid_slots` 硬约束，从源头避免冲突；导出全部课表不再做事后校验。这一层不是复用 `verify()`——不同年级作息形状不同，"是否撞车"要按**真实钟点区间**判断，不能比较"第几节"，这条判断逻辑本身没变，只是触发时机从"导出前"挪到了"求解前"。
- **拖拽调整的可视化反馈**：讨论中一度定过"拖起时前端实时算合法/冲突落点"（绿/红预览），后来用户明确改主意——**不做前端实时判断**，沿用现有"确认后交后端 `adjust.py` 算、只退肇事者"机制，"回退提示按违规类型着色"已实现（子项目6）。不要重新捡起前端实时预览这个方向。
- **AI 供应商**：两者并存，**OpenAI 兼容协议为主**（用户自填 base_url + api_key + 模型名），Anthropic 保留为可选项。`ai/rule_parser.py`、`ai/reviewer.py` 两处调用都走同一层抽象（`scheduler/ai/client.py`，子项目8已实现）。
- **年级/班级**：年级名称完全自定义，数量不限（不是只支持初一/初二/初三这个固定集合）。
- **课程/学科系**：按年级分别维护，不是全局一份 `courses.yaml`（子项目2已实现，改动覆盖 `compiler.py`/`verifier.py`/`precheck.py`/`exporter.py`/`importer.py`/`routes.py` 全部读 `cfg.courses` 的地方）。
- **任课表 vs 排课说明.xlsx 的分工**：任课表（矩阵）是"谁教谁"的唯一来源；排课说明.xlsx 降级为纯规则文本表（不能排课节次/固定节次/排课要求/备注），按教师姓名匹配，不再提供班级/课程/周课时——那些改由"课程与学科系"步骤里的课程计划提供（子项目4/5已实现）。现有 `merge_teaching_and_rules` 的"两份 Excel 冲突检测"机制在新流程里作为"补充/过渡视图"降级保留（`ImportPanel.vue`），不再是主路径。
- **规则页面**：现有 `RulesSettings.vue` 直接把 `scheduler/core/rules.py` 的英文 DSL 类型名（`forbid_slots`/`daily_min`……）暴露给用户，教务看不懂。**2026-08-28 做了轻量翻译**（不是原计划里"整体砍掉重做成白话输入项"的完整方案，那个仍未做）：类型下拉框改成中文短标签（`forbid_slots`→"禁排时段"等），未实现的三种类型（`preferred_periods`/`avoid_after`/`teacher_balance`，只在 `RULE_TYPES` 里声明过、compiler.py/verifier.py 从未实现）如实标注"（未实现）"而不是隐藏或假装生效；每条规则卡片下方新增一行只读的完整中文描述（复用后端 `rules.py::describe()`，新增到 `RuleItem.description` 字段，`GET/PUT /api/config/rules` 都会返回，前端不用自己拼模板）——这行文字是"上次保存时"的内容，改参数后要点保存才刷新，不是逐字符实时预览。参数本身仍是原始 JSON 文本框，没有为每种类型单独做表单（12+种类型形状差异太大，值不值得做见 M5 那次的同类判断）。它仍然是「临时视图」，跟 `RulesSheetSettings.vue`（子项目5，自动生成大部分规则）配合使用，`App.vue` 里的 alert 说明依旧准确。
...
| 7. 跨年级统一校验 + 全部导出 | 导出前按真实钟点比对教师跨年级冲突；"导出全部课表" | **已完成**（2026-08-28），**设计随后被子项目9取代**——原实现：新增 `scheduler/core/cross_grade.py::find_cross_grade_conflicts(entries)`，按教师姓名 + 真实钟点区间判断跨年级冲突；`POST /api/export/all/check`（只校验）+ `POST /api/export/all`（校验+导出，冲突时 400 拒绝）；前端 `ExportAllSettings.vue` 有"校验"按钮显示冲突。**开发过程中发现并修复一个真实存在的 bug**（不是这次新引入的，独立于后面的设计变更，依然成立）：`ws.py` 的 `_run_job`（`/api/solve` 这条真正被产品使用的求解路径）重建 `Dataset` 时完全没有传 `calendar=` 参数，一直在用 `models.py` 的硬编码默认日历（9节/天、无 `clock_times`、无 `reserved_slots`）——`cli.py`、`import_teaching_table` 等路径一直是对的（都显式调用 `cfg.calendar_of(grade)`），只有 `_run_job` 从 `teaching.yaml` 原始字典直接重建 Dataset 这条路径漏了。对初三而言形状恰好一样（9节/午休5节后）掩盖了问题；对七年级（8节/天）会静默用错误的网格求解，且 reserved_slots 挖空和真实钟点表全部失效。已修复为 `Dataset(..., calendar=cfg.calendar_of(data['grade']))`——**这个修复是子项目9求解阶段避让能生效的前提**（需要 `job.dataset.calendar.clock_times` 是真的），继续保留。同一天晚些时候，用户明确要求把"导出前事后校验"改成"求解阶段就避开"，`find_cross_grade_conflicts`/`/api/export/all/check`/前端"校验"按钮均已删除，详见子项目9 |
| 8. AI 供应商抽象 | OpenAI 兼容为主 + Anthropic 并存 | **已完成**（2026-08-28）——新增 `scheduler/ai/client.py`：`get_ai_client()` 按「设置 → AI 设置」里选的供应商（`ai.provider`，默认 `openai`）构造客户端，`ai/rule_parser.py::parse_row_ai` 和 `ai/reviewer.py::review_schedule` 两处调用方都改成只依赖统一接口 `client.complete(system, user, max_tokens)`，不再直接碰某个供应商 SDK 的形状。OpenAI 兼容协议**不引入 openai SDK**——直接用已有依赖 `httpx` 打标准 `{base_url}/chat/completions` HTTP 端点，因为"OpenAI 兼容"的第三方/自建服务五花八门，裸 HTTP 比依赖某个 SDK 客户端的行为更稳；Anthropic 继续用 anthropic SDK。两个供应商的凭据分开存储（`ai.openai.base_url`/`ai.openai.api_key`/`ai.openai.model` vs 历史上就有的 `ai.api_key`，不改名不迁移），切换供应商不会互相覆盖，回切回去不用重新填。API 端点重做：`GET /api/settings/ai` 同时报两边的配置状态（不能只看当前选中的那个，用户切换 tab 时要立刻看到"另一个供应商是不是已经配过"）；`PUT` 按 `provider` 字段只更新那个供应商的字段，字段留空表示"不改这一项"；`POST /test` 通过 `get_ai_client()` 走统一接口测试当前**已保存**的配置。前端 `AiSettings.vue` 重做成 tab 切换（OpenAI 兼容协议 tab 默认选中，主推；Anthropic tab 次选），每个 tab 已配置时显示"已配置"徽章。**改动波及既有测试**：`test_ai_rule_parser.py`/`test_ai_reviewer.py`/`test_importer_merge.py` 里所有注入 AI 客户端的 FakeClient 都是模拟旧的 anthropic SDK 形状（`client.messages.create()`），全部改造成模拟新的统一接口（`client.complete()`），`test_api_settings.py` 端点测试整体重写为双供应商场景（含"切换供应商不清空另一边"这条关键断言）。新增 `test_ai_client.py`（7项，覆盖默认供应商选择、两个供应商各自的缺配置报错、OpenAI 兼容客户端的 HTTP 调用/解析/异常处理）。浏览器实测确认两个 tab 都正确渲染、切换不报错，"测试连接"在两边都未配置时正确报出"OpenAI 兼容协议未配置完整"（因为没点保存、后端读的是已持久化的默认供应商，不是未保存的草稿——符合设计，没有污染本机真实的 `~/.scheduler/scheduler.db`）。443 项后端测试、138 项前端测试全绿 |
| 9. 跨年级冲突改为求解阶段避让（设计变更，取代子项目7的事后校验） | 用户明确要求："导出前事后校验不做了，求解阶段就解决吧" | **已完成**（2026-08-28）——`cross_grade.py` 整个重写：`find_cross_grade_conflicts`（事后检测）删除，新增 `compute_cross_grade_lock_rules(grade, calendar, other_entries)`（事前避让）：把其它年级的已排时段换算成本年级坐标系下的 `forbid_slots` 规则。"其它年级已经排的是哪一份"这个关键设计点跟用户确认过——**自动取每个其它年级最近一次求解（`sessions.list_jobs()` 按 `created_at DESC`）的第一个候选方案**，不要求用户手动指定基准（用户认可这个方案，权衡过"更精确但每次求解前多一步"的手动做法，选了跟"求解是交互级操作"体验更配的自动方案）。`ws.py` 的 `_run_job` 在 `load_rules()` 之后、编译之前，插入这批跨年级锁定规则一起送进 `precheck`/`compile_model`——教师因此产生的"可用格子减少"跟教师个人禁排是同一套 forbid_slots 机制，L1 预检"需要N节，可用M格，缺X格"的既有诊断消息不用改代码就能自然覆盖这种情况，没有另外做专门的错误文案（YAGNI）。`/api/export/all` 简化成直接打包 zip，不再做导出前校验；`/api/export/all/check` 端点、`ExportAllCheckResponse`/`CrossGradeConflictItem` schema、前端"校验"按钮和冲突列表全部删除。**顺带完成用户同一条消息里提的另一件事**："规则里面那一堆英文记得翻译"——`RulesSettings.vue` 的规则类型下拉框原来直接显示 `forbid_slots`/`daily_min` 这类内部 DSL 英文名，加了 `TYPE_LABEL` 中文映射（13 种类型全覆盖，其中 `preferred_periods`/`avoid_after`/`teacher_balance` 三种其实从未被 compiler.py/verifier.py 实现过，如实标注"（未实现）"而不是隐藏，顺手绕过一个潜在的用户会选中一个完全不生效的规则类型的坑）；同时给 `RuleItem` 加了 `description` 字段，后端 `GET/PUT /api/config/rules` 复用已有的 `rules.py::describe()` 直接把每条规则的完整中文说明算好传给前端，规则卡片下方多一行"[年级=初三] 教师半天连堂不超过 2 节"这样的只读预览（上次保存时的内容，不是逐字符实时预览）。测试：`test_cross_grade.py` 从测"检测冲突"改成测"生成避让规则"（6项，沿用两套不同作息形状的日历验证换算逻辑）；`test_api_export_all.py` 重写，核心新增 `test_solving_a_second_grade_avoids_the_teachers_already_committed_slot`——**真正走通真实 `/api/solve` + WebSocket 全流程**（不是单测 `compute_cross_grade_lock_rules`）：两个年级共用同一份教师，先后调用真实求解 API，断言第二个年级最终解出来的时段跟第一个年级不一样，是这次改动里最有说服力的一条测试；`RulesSettings.spec.ts` 新增 2 项（类型下拉中文化、description 预览渲染）。浏览器实测确认 `/api/config/rules` 真实返回 `description` 字段且下拉框 13 个选项全部显示中文标签（含"未实现"标注），"导出全部课表"页面的"校验"按钮已经从界面上消失。443 项后端测试、138 项前端测试全绿 |
...
## 去掉硬编码9节/天（2026-09-01）

用户明确要求："一定要去掉硬编码9节/天，具体一天的时间是根据作息表（时间表）绝定"。逐一排查后找到的真实硬编码点、以及每处的处理方式：

- **`ai/rule_parser.py` 的 AI 提示词**——之前写死"节次用1-9，第9节是下午第4节"，对七年级（8节/天，午休第4节后）这种不同形状的年级会让 AI 按错误的偏移换算下午节次。`parse_row_ai()` 新增 `calendar` 参数，`_period_desc(calendar)` 按传入日历动态生成换算说明；两处调用方（`merge_teaching_and_rules`/`import_rule_text_table`）都已经有 `calendar` 在作用域里，直接传。测试新增 `test_prompt_reflects_the_grades_real_calendar_not_a_hardcoded_shape`，对比 9 节和 8 节两种日历生成的提示词文本确实不同。
- **`solver.py` 的 `Placement.day`/`.period` 属性**——依赖一个遗留的全局 `calendar.py` 模块（同样硬编码9节），且已确认是死代码（grep 全项目没有任何地方调用这两个属性，真正的换算入口一直是 `GradeCalendar.slot_of()`，每个 Dataset 带自己的 calendar）。属性和对应的 `from . import calendar as cal` 导入直接删除。
- **前端课表格子组件**——`ScheduleGrid.vue`/`TeacherScheduleGrid.vue`/`VenueOccupancyGrid.vue` 其实早就支持传入真实的 `days`/`periodsPerDay`（更早的会话就做了），但 `CandidateTabs.vue` 一直没有真正去取某个年级的真实作息表传下去，一直在悄悄用默认值9兜底。补上：`CandidateTabs.vue` 按 `props.grade` 调 `getCalendar()`，取不到（还没配）就让子组件继续走默认值，不阻塞渲染。测试新增用 8 节/天日历验证渲染行数是 40 不是 45。

**`scheduler/core/calendar.py`（遗留的全局9节常量模块）没有删除**——排查发现全项目有 9 个测试文件、65 处调用依赖它的 `N_SLOTS`/`slot_index` 等常量来快速构造"默认形状"的测试数据，这些用法本身没有 bug（测试用的就是标准9节/天场景），批量重写这些测试文件属于价值不大的大范围churn。只修正了它的文档字符串（原来写着"全系统唯一的换算入口"，这句话不实——生产代码从未读它），改成如实说明这是"仅供测试构造默认日历数据用的常量，不是生产入口"。

**没有处理、确认过不需要处理**：`scheduler/config/calendar.yaml`（单数，项目最早骨架提交留下的旧文件）本来就不被任何代码读取（`compiler.py` 只在注释提过一句），跟这次的"硬编码"问题无关，见文件里已有的说明。

### 作息表维护页面补全一个手动编辑入口

第2步"作息时间"页面（`CalendarSettings.vue`，更早的会话已经做出批量 Excel 导入 + 各年级状态列表）缺一个"改一点作息不用重新做 Excel"的手动编辑入口。新增：点"手动编辑"（或未配置时的"手动新建"）展开一个表单，每天节数、午休边界、逐节上下课钟点直接改，"保存"复用已有的 `PUT /api/config/calendars/{grade}` 端点（后端本来就支持，不用改）。每天节数改动会自动增减钟点表的行数（`syncClockTimesLength`），不需要手动对齐。测试新增4项覆盖预填充、保存、改节数联动行数、取消不保存。

浏览器实测：真实 32 班配置的初三点开"手动编辑"，9节/午休第5节后的真实数据完整预填充，"取消"确认不落盘（跟真实数据 `git diff` 零残留）。

## M7：三年级合排（核心已完成，2026-09-01；打包 exe 未开始）

原计划的两处前置重构（`TeachingTask.class_id` 裸整数冲突、`importer` task id 每次从 0 开始）**没有按原计划去改**——设计合排方案时发现有一条更简单、风险更小的路径能绕开这两个缺口，细节见下。打包 exe 依然完全没开始，不是这次的目标（用户明确说"打包不着急"）。

### 合排的实际做法：共享一个 CP-SAT 模型，不合并 Dataset

新增 `scheduler/core/merge_solver.py`。核心思路：**不**把多个年级的 `TeachingTask`/`class_id` 合并进同一个 `Dataset` 再喂给 `compile_model`（那样才会真正踩中上面两处缺口）；而是让 `compile_model` 支持"往一个外部传入的 `cp_model.CpModel()` 里追加约束"（新增 `model=`/`finalize=` 两个参数，默认值保持单年级用法完全不变），对每个参与合排的年级**分别**用它自己原本的 `Dataset`（`class_id` 不跨年级混用、任务列表不合并）调用一次 `compile_model`，只是把 `model` 参数指向同一个共享对象。这样每个年级内部的建模逻辑（班级互斥、教师互斥、所有规则）完全不变、天然按年级隔离，`class_id` 裸整数冲突根本不会发生——因为每次 `_add_class_no_clash` 调用看到的 `dataset.tasks` 本来就只有一个年级的。

唯一真正需要处理的是 **task id 全局唯一**（`CompiledModel.x` 字典用 `(task_id, slot)` 做键，共享一个模型后必须全局不撞）：`merge_solver._renumber_tasks()` 在合排开始前把每个年级的 task id 重新编号成全局唯一，同时留一份"新 id → 原 id"的映射；`solve_merged()` 返回结果前用这份映射把 `Placement.task_id` 换回调用方原本认得的编号——下游（exporter、adjust.py、前端）看到的还是每个年级自己一贯的 task id，感知不到内部发生过重编号。这个映射只在 `merge_solver.py` 内部用一次，不是持久化的全局方案，所以"task id 每次从 0 开始"这条缺口本身**依然存在**，只是合排这条路径不需要修它——真要走"合并 Dataset 塞进一个模型"这种更激进的方案时，那条缺口才会重新变得相关。

### 跨年级联动约束：教师互斥 + 共享场地容量，按真实钟点

参与合排的年级作息形状可能不同（七年级8节/天，初三/八年级9节/天），"同一位老师会不会在两个年级撞车"不能比较"第几节"，必须按真实钟点区间判断——跟子项目9的口径一致，但这次是**真正联合求解**，不是"求解阶段单向避让"的近似（子项目9继续保留、不受影响，"各年级独立求解"路径不需要真的合排时依然可用，两条路径并存是用户明确要求的）。

具体做法：`_time_bucket_boundaries()` 把所有参与年级的每节课起止时刻合并去重，得到能同时兼容所有年级作息形状的"最细公共时间格"边界（第 i 个 bucket 是 `[boundaries[i], boundaries[i+1])`，任何年级任何一节课要么完整覆盖某个 bucket、要么跟它完全不相交）；`_slot_buckets()` 预算每个年级每个 slot 覆盖哪些 bucket（只依赖日历，每年级算一次）；`_add_cross_grade_teacher_no_clash()` 按 `(星期, bucket, 教师, 单双周)` 分组收集所有年级的占用布尔量，总和 `<= 1`；`_add_cross_grade_venue_capacity()` 同样分组但按场地统计，总和 `<= capacity`。单年级内部本来就有的同类约束（`_add_teacher_no_clash`/`add_venue_constraints`）在这里被重复加了一遍等价约束，无害（CP-SAT 不介意冗余约束），换来的是不用去判断"这个 bucket 是不是跨了多个年级"这种额外分支逻辑。

**场地容量新增了"按年级单独分配"的配置**（用户原话："场地也做（可以选择不做，或者可以单独设置每个年级几个场地）"）：`Venue` 模型新增 `grade_capacity: Dict[str, int]`，某个年级在这里配了值，这个场地对该年级就**不参与**跨年级共享判定，只在本年级内部按这个数字生效（`add_venue_constraints` 优先读 `grade_capacity.get(grade, capacity)`）——比如操场按年级分区使用时，各年级各自配自己的位置数，互不影响。前端 `CourseSettings.vue` 的"场地容量"表格新增一列"{当前年级}单独分配"，留空表示仍走共享池。

### API 与前端：复用现有的求解任务基础设施，不新建一套

`teaching.yaml` 同一时刻只服务一个年级（多年级操作流程重构的既有约束），合排没法像单年级求解那样重新从 `teaching.yaml` 读——`POST /api/solve/merged`（`ws.py::solve_merged_grades`）复用"该年级最近一次求解"留下的 `Dataset`/规则快照作为教学数据来源，这跟子项目9"取其它年级最近一次结果"是同一个思路，只是这次是每个参与年级都这样取，不分主次。求解前对每个年级各自跑一遍 L1 预检（复用 `precheck()`），任何一个年级没过就整体拒绝、不进求解器。求解结果按年级各自存成一条普通的 `SolveJob`（只有 1 个候选方案）——直接复用现有的历史列表/导出/AI 审核/拖拽调整整套基础设施，没有为合排另建一套 job 类型或持久化表。这是同步阻塞的 HTTP 端点，不是像单年级求解那样走 WebSocket 逐候选推送——合排只产出一个联合可行解（不是"求多个候选方案供选"，那是独立求解 `solve_many` 的定位），且预期跟 0.6 秒基线同量级，没必要为此新增一套进度推送机制（YAGNI）。

前端新增 `MergedSolveSettings.vue`，挂在第 7 步"排课与调整"、`SolvePanel`/`CandidateTabs` 下方：年级复选框（默认全选）+ 最大求解秒数 + "开始合排"按钮，结果按年级列出状态/耗时/违规数，提示"可以照常在历史记录里查看、导出"（没有做"自动跳转到对应年级看课表"这种交互糖，长这个按钮点开后用户还是要自己切年级看，这是可以接受的简化）。

### 测试

`test_merge_solver.py`（8项）覆盖：task id 全局重编号、跨年级教师互斥（用两个作息形状不同的日历确认按真实钟点算、不是巧合对齐"第几节"）、不同教师不受影响、结果 task_id 正确换回调用方原编号、跨年级共享场地容量、`grade_capacity` 让某年级的场地不参与共享、只合排 1 个年级时退化成普通单年级求解（确认合排机制对现有单年级场景零副作用）、合排不会绕过单年级内部原有约束。`test_api_merged_solve.py`（4项）在 API 层面走真实 `/api/solve` + `/api/solve/merged` 全流程，核心测试 `test_merged_solve_avoids_cross_grade_teacher_conflicts_even_when_the_solo_solves_did_not`——两个年级独立求解互不知道对方（可能撞车也可能巧合没撞），合排出来的结果断言真的没有冲突，不是延续独立求解偶然凑巧的结果，这条测试最有说服力。
