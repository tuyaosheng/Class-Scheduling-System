"""9 节/天、周一到周五的默认时间格常量——仅供测试构造默认形状的日历数据
用（等价于 `models.GradeCalendar` 的默认值）。

不是生产代码的换算入口：真正按年级参数化的口径在 `models.GradeCalendar`
（`slot_index`/`slot_of`/`section_period` 等方法），每个 Dataset 携带
自己的 `calendar` 字段，不同年级的节数/午休边界/钟点表都可能不一样。
"""
