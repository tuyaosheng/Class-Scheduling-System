...
def _dataset():
    ...
    老师，制造教师冲突用）、英语。语文周课时 2（同一个 task_id 对应两条
    placement，专门覆盖"一个任务有多节课"的场景）。位置任意——测试里直接
    摆 Placement，不走求解器。"""
    tasks = [
...
def _placements():
    return [
        ...
        Placement(task_id=0, class_id=1, course='语文', teacher='张老师', slot=2, parity=None),
...
def test_clean_move_is_applied_without_revert():
    ...
    # 不撞任何人，应该直接生效。
    result = apply_and_prune(placements, {(1, 1): 20}, _dataset(), _cfg(), [])
...
def test_single_conflicting_move_is_fully_reverted():
    placements = _placements()
    # task 0 的第一节课（slot 0）挪到 slot 10——张老师已经在 slot 10 教
    # 2 班语文（task 2），制造教师分身。
    result = apply_and_prune(placements, {(0, 0): 10}, _dataset(), _cfg(), [])
    assert result.applied == []
...
def test_multi_move_only_reverts_the_culprit():
    ...
    assert by_slot[20].task_id == 1   # 没问题的那个照常生效


def test_entangled_moves_degrade_to_full_revert():
    placements = _placements()
    # task 0 的第一节课（slot 0）和 task 4 都挪到 slot 1——那是 task 1
    # （没在这批改动里、固定不动）已经占着的格子。三个人挤一个格子：
    # 单独退哪个都不够，格子里依然剩 2 个人，违规数原地不动（还是 1
    # 处），贪心找不到"退一个能改善"的选择，只能整体回退两个。
    moves = {(0, 0): 1, (4, 30): 1}
    result = apply_and_prune(placements, moves, _dataset(), _cfg(), [])
    assert result.applied == []
...
def test_moving_one_occurrence_of_a_multi_period_task_leaves_the_other_alone():
    """回归测试：task 0（语文，周课时 2）有两条 placement（slot 0、slot 2）。
    只拖动第一节（slot 0），第二节（slot 2）必须原地不动——不能因为共用
    task_id 就被一起拖走。"""
    placements = _placements()
    result = apply_and_prune(placements, {(0, 0): 20}, _dataset(), _cfg(), [])
    assert result.applied == [(0, 0)]
    assert result.reverted == []

    task0_slots = sorted(p.slot for p in result.placements if p.task_id == 0)
    assert task0_slots == [2, 20]   # 第一节挪到 20，第二节（slot 2）纹丝不动
